<header
    ...
    <div class="hidden md:flex flex-1 justify-end gap-8">
        ...
        @guest
        <a class="flex min-w-[84px] max-w-[480px] cursor-pointer items-center justify-center overflow-hidden rounded-lg h-10 px-4 bg-primary text-white text-base font-bold leading-normal tracking-[0.015em] hover:bg-primary transition-colors'" wire:navigate href="{{ route('login') }}">Login</a>
        @endguest

        @auth
        <livewire:auth.logout wire:navigate />
        @endauth
    </div>
</header>
